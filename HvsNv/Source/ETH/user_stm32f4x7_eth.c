...
static U32 WritePHYRegister(U16 PHY_AbsoluteAddr,U16 PHY_Data)
{
	return(ETH_WritePHYRegister((PHY_AbsoluteAddr >> 5)&0x1f,PHY_AbsoluteAddr&0x1f,PHY_Data));
}

static U16 ReadPHYRegister(U16 PHY_AbsoluteAddr)
{
	return(ETH_ReadPHYRegister((PHY_AbsoluteAddr >> 5)&0x1f,PHY_AbsoluteAddr&0x1f));
}

extern U32 UserInitPHY(ETH_InitTypeDef* ETH_InitStruct)
{
	U16 PortStatus;
	U16 Port2MAC_Ctrl;
	U16 PortRegister;
	U8 ConnectPort = 0;
	const U8 CONNECT_PORT_0 = 1;
	const U8 CONNECT_PORT_1 = 2;
	U32 TimeOut;
	
	WritePHYRegister(PHY_P0_BASIC_MODE_CTRL,PHY_Reset);
	WritePHYRegister(PHY_P1_BASIC_MODE_CTRL,PHY_Reset);
//	//os_dly_wait(1);
	...
	}while (ReadPHYRegister(PHY_P0_BASIC_MODE_CTRL) & PHY_Reset);
	...
	}while (ReadPHYRegister(PHY_P1_BASIC_MODE_CTRL) & PHY_Reset);
	
	//Switch复位
	WritePHYRegister(SWH_SWITCH_RESET,3);
	TimeOut = 0;
	do
	{
		TimeOut++;
	...
	}while (ReadPHYRegister(SWH_SWITCH_RESET) & 0x0003);
	...
	}while (!(ReadPHYRegister(PHY_P0_BASIC_MODE_STATUS) & PHY_Linked_Status));
	//Port 0已建立连接
	if(TimeOut < 0xFFFF)
	{
		ConnectPort |= CONNECT_PORT_0;
	}
	//检测PORT1链路是否建立
	TimeOut = 0;
	do
	...
	}while (!(ReadPHYRegister(PHY_P1_BASIC_MODE_STATUS) & PHY_Linked_Status));
	//Port1已建立连接
	if(TimeOut < 0xFFFF)
	{
		ConnectPort |= CONNECT_PORT_1;
	}
	
	//禁止地址学习，不然IN和OUT的两根网线互换，会出现需要无法通信现象
	PortRegister = ReadPHYRegister(SWH_P0_BASIC_CTRL1);
	PortRegister |= 0x7000;
	WritePHYRegister(SWH_P0_BASIC_CTRL1,PortRegister);
		
	PortRegister = ReadPHYRegister(SWH_P1_BASIC_CTRL1);
	PortRegister |= 0x7000;
	WritePHYRegister(SWH_P1_BASIC_CTRL1,PortRegister);
		
	PortRegister = ReadPHYRegister(SWH_P2_BASIC_CTRL1);
	PortRegister |= 0x7000;
	WritePHYRegister(SWH_P2_BASIC_CTRL1,PortRegister);
		
	Port2MAC_Ctrl = ReadPHYRegister(SWH_P2_MAC_CTRL);
	
	//利用port0进行自适应设置
	if(ConnectPort & CONNECT_PORT_0)
	{
		//使能PORT0自适应
		WritePHYRegister(PHY_P0_BASIC_MODE_CTRL,PHY_AutoNegotiation);
		//等待PORT0自适应完成
		TimeOut = 0;
		do
		{
		...
		}while (!(ReadPHYRegister(PHY_P0_BASIC_MODE_STATUS) & PHY_AutoNego_Complete));
		//自适应成功，没有超时
		if(TimeOut < 0xFFFF)
		{
			PortStatus = ReadPHYRegister(SWH_P0_STATUS);
			if(PortStatus & SWH_FULL_DUPLEX)
			{
				ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;
				//Port2 即RMII设为全双工模式
				Port2MAC_Ctrl &= 0xfffd;				
			}
			else
			{
			...
			if(PortStatus & SWH_100MBPS)
			{
				//Port2 即RMII设为100M bps
				ETH_InitStruct->ETH_Speed = ETH_Speed_100M;
				Port2MAC_Ctrl &= 0xfffe;		
			}
			else
			{
				ETH_InitStruct->ETH_Speed = ETH_Speed_10M;
				//Port2 即RMII设为10M bps
				Port2MAC_Ctrl |= 0x0001;
			}
			WritePHYRegister(SWH_P2_MAC_CTRL,Port2MAC_Ctrl);
			WritePHYRegister(PHY_P1_BASIC_MODE_CTRL,PHY_FULLDUPLEX_100M);
			return(ETH_SUCCESS);			
		}
	}
	//利用port1进行自适应设置
	else if(ConnectPort & CONNECT_PORT_1)
	{
		//使能PORT1自适应
		WritePHYRegister(PHY_P1_BASIC_MODE_CTRL,PHY_AutoNegotiation);
		//等待PORT1自适应完成
		TimeOut = 0;
		do
		{
		...
		}while (!(ReadPHYRegister(PHY_P1_BASIC_MODE_STATUS) & PHY_AutoNego_Complete));
		//自适应成功，没有超时
		if(TimeOut < 0xFFFF)
		{
			PortStatus = ReadPHYRegister(SWH_P1_STATUS);
			if(PortStatus & SWH_FULL_DUPLEX)
			{
				ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;
				Port2MAC_Ctrl &= 0xfffd;				
			...
			WritePHYRegister(SWH_P2_MAC_CTRL,Port2MAC_Ctrl);
			WritePHYRegister(PHY_P0_BASIC_MODE_CTRL,PHY_FULLDUPLEX_100M);
			return(ETH_SUCCESS);	
		}
	}
	
	//port0和port1都没有建立连接，或者自适应超时，则设置为100M全双工	
	WritePHYRegister(PHY_P0_BASIC_MODE_CTRL,PHY_FULLDUPLEX_100M);
	WritePHYRegister(PHY_P1_BASIC_MODE_CTRL,PHY_FULLDUPLEX_100M);
	ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;
	Port2MAC_Ctrl &= 0xfffd;	
...
//	} while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status) && (timeout < PHY_READ_TO1));

    /* Return ERROR in case of timeout */
    if(timeout == PHY_READ_TO)
//	if(timeout == PHY_READ_TO1)
    {
      return ETH_ERROR;
    }
//	else{
...
//	}
  ...
  if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
  {
	  ...
	  err = UserInitPHY(ETH_InitStruct);
	  if(err == ETH_ERROR)
	  {
		  goto error;
	  }
  ...
  {
	  status0 = ETH_WritePHYRegister(DM8603_PHY0_ADDR,PHY_BCR,((uint16_t)(ETH_InitStruct->ETH_Mode >> 3) |
                                                  (uint16_t)(ETH_InitStruct->ETH_Speed >> 1)));
	  status1 =  ETH_WritePHYRegister(DM8603_PHY1_ADDR,PHY_BCR,((uint16_t)(ETH_InitStruct->ETH_Mode >> 3) |
                                                   (uint16_t)(ETH_InitStruct->ETH_Speed >> 1)));
	  if((status0 == ETH_ERROR) || (status1 == ETH_ERROR))
	  {
		 /* Return ERROR in case of write timeout */
		err = ETH_ERROR;
	  }
